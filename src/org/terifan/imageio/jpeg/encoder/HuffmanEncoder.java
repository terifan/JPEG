...
public static class JHUFF_TBL
{
  /* These two fields directly represent the contents of a JPEG DHT marker */
  int[] bits = new int[17];		/* bits[k] = # of symbols with codes of */
				/* length k bits; bits[0] is unused */
  int[] huffval = new int[256];		/* The symbols, in order of incr code length */
...
{
  int[] ehufco = new int[256];	/* code for each symbol */
  int[] ehufsi = new int[256];		/* length of code for each symbol */
...
{
  int put_buffer;		/* current bit-accumulation buffer */
...
static class huff_entropy_encoder extends JPEGEntropyState
{
   ...
   */
  int[] next_output_byte;	/* => next byte to write in buffer */
  int next_output_byte_offset;
  int free_in_buffer;	/* # of byte spaces remaining in buffer */
  JPEG cinfo;		/* link to cinfo (needed for dump_buffer) */

  /* Coding status for AC components */
  int ac_tbl_no;		/* the table number of the single component */
  int EOBRUN;		/* run length of EOBs */
  int BE;		/* # of buffered correction bits before MCU */
  int[] bit_buffer;		/* buffer for correction bits (1 per char) */
...
static class working_state
{
  ...
  JPEG cinfo;		/* dump_buffer needs access to this */
}
...
int MAX_CORR_BITS = 1000;	/* Max # of correction bits I can buffer */
...
private boolean emit_byte_s(working_state state,int val)  
	{
		state.next_output_byte[state.next_output_byte_offset++] = val;  
	  if (--state.free_in_buffer == 0)  
	    return dump_buffer_s(state);
	  return true;
  } 

/* Emit a byte */
void emit_byte_e(huff_entropy_encoder entropy, int val)  
	{ 
		entropy.next_output_byte[entropy.next_output_byte_offset++] = val;  
...
void dump_buffer_e (huff_entropy_encoder entropy)
...
boolean emit_bits_s (working_state state, int code, int size) /* Emit some bits; return TRUE if successful, FALSE if must suspend */
{
  ...
  while (put_bits >= 8) {
    ...
    if (!emit_byte_s(state, c)) return false;
    if (c == 0xFF) {		/* need to stuff a zero byte? */
      if (!emit_byte_s(state, 0)) return false;
    }
    put_buffer <<= 8;
    put_bits -= 8;
  }
...
void emit_bits_e (huff_entropy_encoder entropy, int code, int size)
...
void 
flush_bits_e (huff_entropy_encoder entropy)
{
...
void emit_dc_symbol (huff_entropy_encoder entropy, int tbl_no, int symbol)
{
  if (entropy.gather_statistics)
...
void emit_ac_symbol (huff_entropy_encoder entropy, int tbl_no, int symbol)
{
  if (entropy.gather_statistics)
...
void emit_buffered_bits (huff_entropy_encoder entropy, int[] bufstart, int bufoffset, int nbits)
{
  if (entropy.gather_statistics)
    return;			/* no real work */
...
void emit_eobrun (huff_entropy_encoder entropy)
{
  int temp, nbits;
...
boolean emit_restart_s (working_state state, int restart_num)
{
  ...
  if (!emit_byte_s(state, 0xFF)) return false;
  if (!emit_byte_s(state, RST0 + restart_num))return false;
...
void emit_restart_e (huff_entropy_encoder entropy, int restart_num)
{
  int ci;
...
boolean encode_mcu_DC_first (JPEG cinfo, int[][] MCU_data)
{
...
boolean encode_mcu_AC_first (JPEG cinfo, int[][] MCU_data)
{
  huff_entropy_encoder entropy = (huff_entropy_encoder) cinfo.entropy;
  int[] natural_order;
  int[] block;
...
boolean encode_mcu_DC_refine (JPEG cinfo, int[][] MCU_data)
{
  huff_entropy_encoder entropy = (huff_entropy_encoder) cinfo.entropy;
  int Al, blkn;
...
boolean encode_mcu_AC_refine (JPEG cinfo, int[][] MCU_data)
{
  huff_entropy_encoder entropy = (huff_entropy_encoder) cinfo.entropy;
  int[] natural_order;
  int[] block;
...
boolean encode_one_block(working_state state, int[] block, int last_dc_val,		  c_derived_tbl dctbl, c_derived_tbl actbl)
{
  int temp, temp2;
  int nbits;
  int r, k;
...
boolean encode_mcu_huff (JPEG cinfo, int[][] MCU_data)
{
  huff_entropy_encoder entropy = (huff_entropy_encoder) cinfo.entropy;
...
@Override
public void finish_pass(JPEG cinfo, boolean gather_statistics)
{
...
void finish_pass_huff(JPEG cinfo)
{
  huff_entropy_encoder entropy = (huff_entropy_encoder) cinfo.entropy;
  working_state state = new working_state();
...
void jpeg_gen_optimal_table (JPEG cinfo, JHUFF_TBL htbl, int[] freq)
{
...
void finish_pass_gather (JPEG cinfo)
{
  huff_entropy_encoder entropy = (huff_entropy_encoder) cinfo.entropy;
  int ci, tbl;
  ComponentInfo compptr;
...
//  MEMZERO(did_ac, SIZEOF(did_ac));
  ...
  for (ci = 0; ci < cinfo.comps_in_scan; ci++) {
	...
	if (htblptr == null)
	  htblptr = cinfo.dc_huff_tbl_ptrs[tbl] = new JHUFF_TBL();
	...
	if (htblptr == null)
	  htblptr = cinfo.ac_huff_tbl_ptrs[tbl] = new JHUFF_TBL();
...
//    if (gather_statistics)
//      entropy.encode_mcu = encode_mcu_gather;
//    else
//      entropy.encode_mcu = encode_mcu_huff;
...
//  entropy.start_pass = start_pass_huff;
