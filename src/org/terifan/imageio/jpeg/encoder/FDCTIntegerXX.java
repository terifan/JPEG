...
public class FDCTIntegerXX
{
	...
	public void forward(int[] elemptr)
	{
		int[] workspace = new int[64];

		for (int ctr = 0; ctr < 64; ctr += 8)
		{
			int tmp0 = elemptr[ctr + 0] + elemptr[ctr + 7];
			int tmp1 = elemptr[ctr + 1] + elemptr[ctr + 6];
			int tmp2 = elemptr[ctr + 2] + elemptr[ctr + 5];
			int tmp3 = elemptr[ctr + 3] + elemptr[ctr + 4];
			...
			tmp0 = elemptr[ctr + 0] - elemptr[ctr + 7];
			tmp1 = elemptr[ctr + 1] - elemptr[ctr + 6];
			tmp2 = elemptr[ctr + 2] - elemptr[ctr + 5];
			tmp3 = elemptr[ctr + 3] - elemptr[ctr + 4];
		...
		for (int ctr = 0; ctr < 8; ctr++)
		{
			...
			workspace[8 * 0 + ctr] = RIGHT_SHIFT(tmp10 + tmp11, PASS1_BITS);
			workspace[8 * 4 + ctr] = RIGHT_SHIFT(tmp10 - tmp11, PASS1_BITS);

			int z1 = MULTIPLY(tmp12 + tmp13, FIX_0_541196100);
			z1 += 1 << (CONST_BITS + PASS1_BITS - 1);
			workspace[8 * 2 + ctr] = RIGHT_SHIFT(z1 + MULTIPLY(tmp12, FIX_0_765366865), CONST_BITS + PASS1_BITS);
			workspace[8 * 6 + ctr] = RIGHT_SHIFT(z1 - MULTIPLY(tmp13, FIX_1_847759065), CONST_BITS + PASS1_BITS);
			...
			workspace[8 * 1 + ctr] = RIGHT_SHIFT(tmp0 + tmp10 + tmp12, CONST_BITS + PASS1_BITS);
			workspace[8 * 3 + ctr] = RIGHT_SHIFT(tmp1 + tmp11 + tmp13, CONST_BITS + PASS1_BITS);
			workspace[8 * 5 + ctr] = RIGHT_SHIFT(tmp2 + tmp11 + tmp12, CONST_BITS + PASS1_BITS);
			workspace[8 * 7 + ctr] = RIGHT_SHIFT(tmp3 + tmp10 + tmp13, CONST_BITS + PASS1_BITS);
		}
	}
